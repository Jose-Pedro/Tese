...
legend(h,'Signature', g1, g2, g3, g4, g5, g6);
        ...
        vectors=resizeVector([ts ; vectors'],aux);
...
hold on
h(3)=plot(t,vector2(2,:),'r-x');
hold on; 
h(4)=plot(t,vector3(2,:),'m-.');
hold on; 
h(5)=plot(t,vector4(2,:),'g-o');
hold on; 
h(6)=plot(t,vector5(2,:),'c-+');
hold on; 
h(7)=plot(t,vector6(2,:),'k-s');
hold on; 
grid on
ylabel('joiint values');
